...
public class EmpleadosEmpresa1 {
    ...
    public void cargarEmpleadosDesdeDB() {
        ...
        try (Connection conexion = obtenerConexion(); Statement miStatement = conexion.createStatement()) {
    ...
    private static void añadirEmpleadoDB(Empleado empleado) {
        String sql = "INSERT INTO Empleados (nombre, apellidos, fechaNacimiento, fechaIngreso, puesto, salario) VALUES ('"
            ...
            + Date.valueOf(empleado.getFechaNacimiento()) + "', '"
            + Date.valueOf(empleado.getFechaIngreso()) + "', '"
            + empleado.getPuesto() + "', "
            + empleado.getSalario() + ")";
        try (Connection conexion = obtenerConexion(); Statement miStatement = conexion.createStatement()) {
            int filasCambiadas = miStatement.executeUpdate(sql);
            if (filasCambiadas > 0) {
    ...
    private static void eliminarEmpleadoDB(String nombre, String apellidos) {
        String sql = "DELETE FROM Empleados WHERE nombre = '" + nombre + "' AND apellidos = '" + apellidos + "'";
        try (Connection conexion = obtenerConexion(); Statement miStatement = conexion.createStatement()) {
            int filasCambiadas = miStatement.executeUpdate(sql);
            if (filasCambiadas > 0) {
    ...
    private static void actualizarEmpleadoDB(String nombre, String apellidos, Empleado empleadoNuevo) {
        String sql = "UPDATE Empleados SET nombre = '" + empleadoNuevo.getNombre()
            + "', apellidos = '" + empleadoNuevo.getApellidos()
            + "', fechaNacimiento = '" + Date.valueOf(empleadoNuevo.getFechaNacimiento())
            + "', fechaIngreso = '" + Date.valueOf(empleadoNuevo.getFechaIngreso())
            + "', puesto = '" + empleadoNuevo.getPuesto()
            + "', salario = " + empleadoNuevo.getSalario()
    ...
    private static void buscarEmpleadoDB(String nombre, String apellidos) {
        String sql = "SELECT * FROM Empleados WHERE nombre = '" + nombre + "' AND apellidos = '" + apellidos + "'";
        try (Connection conexion = obtenerConexion(); Statement miStatement = conexion.createStatement()) {
            ResultSet rs = miStatement.executeQuery(sql);
            if (rs.next()) {
        ...
        do {
            ...
            try {
                ...
                if (fechaIngreso != null && fechaNacimiento != null && fechaIngreso.isBefore(fechaNacimiento)) { // Verifico que la fecha de ingreso no sea anterior a la fecha de nacimiento
                    System.err.println("La fecha de ingreso no puede ser anterior a la fecha de nacimiento. Por favor, introduce una fecha válida.");
